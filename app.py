...
def run_shap(model, input_tensor, y, x, shap_samples, use_gpu=False):
    ...
    try:
        if use_gpu and torch.cuda.is_available():
            pixel_model = PixelOutputModel(model, y, x).to(DEVICE)
            input_tensor_shap = input_tensor
            background = torch.zeros_like(input_tensor_shap)
        else:
            cpu_model = UNet(n_channels=1, n_classes=1).cpu()
            cpu_model.load_state_dict(model.state_dict())
            cpu_model.eval()
            pixel_model = PixelOutputModel(cpu_model, y, x).cpu()
            input_tensor_shap = input_tensor.detach().cpu()
            background = torch.zeros_like(input_tensor_shap)

        explainer = shap.GradientExplainer(pixel_model, background)
        shap_values = explainer.shap_values(input_tensor_shap, nsamples=shap_samples)
        return shap_values, "SHAP (GradientExplainer)"
    except Exception as e:
        st.warning(f"SHAP failed: {str(e)}. Using Integrated Gradients fallback.")
